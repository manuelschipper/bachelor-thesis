...
class Generator(object):
	...
	delay : float
		start after delay time"""
	def __init__ (self, env, id, interarr, finish = float("inf"), delay= 0):
		self.id = id
		self.env = env
	...
	def run(self):
		while True:
			...
			if self.rec_csv:
				...
				complete_filename = os.path.join(save_path, self.csv_name + '.csv')
				target = open(complete_filename, 'a')
				try:
					writer = csv.writer(target)
					writer.writerow((wip.id, self.id, start_time, end_time, total_time))
				except:
					raise Exception("Error writing data into CSV File")
				finally:
					target.close()
	...
	def run(self):
		while True:
			...
			for i, j, in zip(self.generators, self.sinks):
				total = self.generators[i].jobs_sent - self.sinks[j].storage
